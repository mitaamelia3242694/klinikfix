...
<section class="blank-content">
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $rekams->firstItem() + $index }}</td>
                <td>{{ $rekam->pasien->nama }}</td>
                <td>{{ \Carbon\Carbon::parse($rekam->tanggal_kunjungan)->format('d-m-Y') }}</td>
                <td>{{ $rekam->keluhan }}</td>
                <td>{{ $rekam->diagnosa ?? '-' }}</td>
                <td>{{ $rekam->tindakan->jenis_tindakan ?? '-' }}</td>
                <td>{{ $rekam->catatan_tambahan ?? '-' }}</td>
                <td>
                    <div style="display: flex; gap: 0.4rem; flex-wrap: wrap;">
                        <a href="{{ route('rekam-medis.show', $rekam->id) }}" class="btn btn-info no-underline"><i
                                class="fas fa-eye"></i></a>
                        <a href="{{ route('rekam-medis.edit', $rekam->id) }}" class="btn btn-warning no-underline"><i
                                class="fas fa-pen"></i></a>
                        <form action="{{ route('rekam-medis.destroy', $rekam->id) }}" method="POST"
    ...
    <ul class="pagination">
        {{-- Previous Page Link --}}
        @if ($rekams->onFirstPage())
        <li class="disabled"><span>«</span></li>
        @else
        <li><a href="{{ $rekams->previousPageUrl() }}" rel="prev">«</a></li>
        @endif

        {{-- Pagination Elements --}}
        @foreach ($rekams->links()->elements[0] as $page => $url)
        @if ($page == $rekams->currentPage())
        ...
        {{-- Next Page Link --}}
        @if ($rekams->hasMorePages())
        <li><a href="{{ $rekams->nextPageUrl() }}" rel="next">»</a></li>
        @else
        <li class="disabled"><span>»</span></li>
        @endif
    </ul>
