...
@section('title', 'Manajemen Pegawai - Klinik')
...
<section class="blank-content">
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="{{ route('data-pegawai.show', $item->id) }}" class="btn btn-info no-underline"
                        style="margin-right: 5px;"><i class="fas fa-eye"></i></a>

                    <a href="{{ route('data-pegawai.edit', $item->id) }}" class="btn btn-warning no-underline"
                        style="margin-right: 5px;"><i class="fas fa-pen"></i></a>

                    <form action="{{ route('data-pegawai.destroy', $item->id) }}" method="POST"
                        style="display:inline-block;" onsubmit="return confirm('Yakin ingin menghapus pegawai ini?');">
                        @csrf
                        @method('DELETE')
    ...
    <ul class="pagination">
        ...
        @else
        <li><a href="{{ $pegawai->previousPageUrl() }}" rel="prev">«</a></li>
        @endif

        {{-- Pagination Elements --}}
        @foreach ($pegawai->links()->elements[0] as $page => $url)
        ...
        <li><a href="{{ $pegawai->nextPageUrl() }}" rel="next">»</a></li>
        @else
        <li class="disabled"><span>»</span></li>
        @endif
    </ul>
    ...
    <div id="modalTambah" onclick="if(event.target === this) this.style.display='none'"
        ...
        <div
            style="background:#fff; padding:2rem; border-radius:12px; width:90%; max-width:720px; max-height:90vh; overflow:auto; box-shadow:0 5px 20px rgba(0,0,0,0.2); position:relative;">
            ...
            <h3 style="margin-bottom:1rem; color:rgb(33, 106, 178); text-align:left;"><i class="fas fa-user-plus"></i>
                Tambah Pegawai</h3>
            ...
            <form method="POST" action="{{ route('data-pegawai.store') }}">
                ...
                <input type="text" name="nik" required
                    style="width:100%; margin-bottom:0.6rem; padding:0.6rem; border:1px solid #ccc; border-radius:6px;">
                ...
                <textarea name="alamat" rows="2" required
                    style="width:100%; margin-bottom:0.6rem; padding:0.6rem; border:1px solid #ccc; border-radius:6px;"></textarea>
                ...
                <input type="text" name="instansi_induk"
                    style="width:100%; margin-bottom:0.6rem; padding:0.6rem; border:1px solid #ccc; border-radius:6px;">
