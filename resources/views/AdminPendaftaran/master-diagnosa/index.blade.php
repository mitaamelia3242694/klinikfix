...
@section('title', 'Master Diagnosa - Klinik')
...
<section class="blank-content">
    ...
    <form method="GET" action="{{ route('master-diagnosa.index') }}"
        style="margin-bottom: 1rem; display: flex; gap: 0.5rem; flex-wrap: wrap;">
        <input type="text" name="keyword" placeholder="Cari diagnosa..." value="{{ request('keyword') }}"
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $diagnosa->nama }}</td>
                <td>
                    <a href="{{ route('master-diagnosa.edit', $diagnosa->id) }}" class="btn btn-warning no-underline"><i
                            class="fas fa-pen"></i></a>
                    <form action="{{ route('master-diagnosa.destroy', $diagnosa->id) }}" method="POST"
    ...
    <div id="modalTambah" onclick="if(event.target === this) this.style.display='none'"
        ...
        <div
            ...
            <form method="POST" action="{{ route('master-diagnosa.store') }}">
                @csrf
