...
@section('title', 'Satuan Obat - Klinik')
...
<section class="blank-content">
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="{{ route('satuan-obat.edit', $satuan->id) }}" class="btn btn-warning no-underline"><i
                            class="fas fa-pen"></i></a>
                    <form action="{{ route('satuan-obat.destroy', $satuan->id) }}" method="POST"
                        style="display:inline-block;" onsubmit="return confirm('Yakin ingin menghapus satuan ini?');">
                        @csrf
                        @method('DELETE')
    ...
    <ul class="pagination">
        ...
        @else
        <li><a href="{{ $satuans->previousPageUrl() }}" rel="prev">«</a></li>
        @endif

        {{-- Page Numbers --}}
        @foreach ($satuans->links()->elements[0] as $page => $url)
        @if ($page == $satuans->currentPage())
        ...
        <li><a href="{{ $satuans->nextPageUrl() }}" rel="next">»</a></li>
        @else
        <li class="disabled"><span>»</span></li>
        @endif
    </ul>
    ...
    <div id="modalTambah" onclick="if(event.target === this) this.style.display='none'"
        ...
        <div
            style="background:#fff; padding:2rem; border-radius:12px; width:90%; max-width:400px; max-height:90vh; overflow:auto; box-shadow:0 5px 20px rgba(0,0,0,0.2); position:relative;">
            ...
            <h3 style="margin-bottom:1rem; color:rgb(33, 106, 178); text-align:left;">Tambah Satuan Obat</h3>

            <form method="POST" action="{{ route('satuan-obat.store') }}">
                @csrf
                <label style="display:block; text-align:left;"><strong>Nama Satuan</strong></label>
                <input type="text" name="nama_satuan" required class="input-style">
