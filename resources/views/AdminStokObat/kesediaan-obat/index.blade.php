...
<section class="blank-content">
    ...
    <div style="margin-bottom: 1rem; padding: 16px; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 12px; border: 1px solid #dee2e6;">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px;">
            ...
            <span id="filterStatus" style="padding: 4px 12px; border-radius: 20px; font-size: 12px; background: #6c757d; color: white;">
                NONAKTIF
            </span>
        </div>
    ...
    <div id="loadingIndicator" style="display: none; text-align: center; padding: 20px; background: #f8f9fa; border-radius: 8px; margin-bottom: 1rem;">
        <div style="display: inline-block; width: 20px; height: 20px; border: 3px solid #f3f3f3; border-top: 3px solid rgb(33, 106, 178); border-radius: 50%; animation: spin 1s linear infinite;"></div>
    ...
    <!-- Summary Information -->
    <div id="summaryBox" style="display: none; margin-top: 1rem; padding: 16px; background: linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%); border-radius: 8px; border-left: 4px solid #2196f3;">
    ...
    <div id="modalTambah" onclick="if(event.target === this) this.style.display='none'"
        ...
        <div style="background:#fff; padding:2rem; border-radius:12px; width:90%; max-width:500px; max-height:90vh; overflow:auto; box-shadow:0 5px 20px rgba(0,0,0,0.2); position:relative;">
...
<script>
    ...
    function filterObat90Hari(sediaans) {
        ...
        sediaans.forEach(sediaan => {
            try {
                ...
                console.log(`Obat: ${sediaan.obat?.nama_obat}, Kadaluarsa: ${sediaan.tanggal_kadaluarsa}, Selisih hari: ${diffDays}`);
    ...
    function renderTable(data) {
        ...
        data.forEach((sediaan, index) => {
            const daysDiff = sediaan.daysDiff !== undefined ? sediaan.daysDiff : calculateDaysDiff(sediaan.tanggal_kadaluarsa);
            const { rowClass, textColor, icon, statusText } = getRowStyle(daysDiff);
            ...
            console.log(`Obat: ${sediaan.obat?.nama_obat}, Stok Total: ${stokTotal}, Jumlah Keluar: ${jumlahKeluar}, Stok Akhir: ${stokAkhir}`);
    ...
    function showSummary(data) {
        ...
        let expired = 0, warning = 0, caution = 0;
